...
namespace Env0.Terminal
{
    ...
    public class NetworkManager
    {
        ...
        public static bool IsIpInSubnet(string ipString, string cidr)
        {
            ...
            {
                var mask = bits >= 8 ? 255 : (bits <= 0 ? 0 : 256 - (1 << (8 - bits)));
        ...
        public List<NmapResult> Nmap()
        {
            ...
            var subnet = CurrentDevice?.Subnet ?? "default";
        ...
        public List<PingResult> Ping(DeviceInfo targetDevice, int packetCount = 4)
        {
            ...
            {
                var dropped = rand.NextDouble() < 0.15; // 15% packet loss
                var ttl = rand.Next(50, 70);             // Random TTL for realism
                var ms = Math.Round(rand.NextDouble() * 100, 1); // up to 100ms
