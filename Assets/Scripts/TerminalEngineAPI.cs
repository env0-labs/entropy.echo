...
namespace Env0.Terminal
{
    public class TerminalEngineAPI
    {
                ...
                case TerminalPhase.Login:
                    ...
                    if (_loginMode == LoginMode.Local)
                    {
                        ...
                        else if (_loginStep == LoginStep.Password)
                        {
                            ...
                            _loginMode = LoginMode.None;
                            _loginStep = LoginStep.None;
                            var warning = string.IsNullOrEmpty(_capturedPassword)
        ...
        private List<string> BuildSessionStackView()
        {
            var stack = new List<string>();
            foreach (var ssh in _session.SshStack)
            {
                // Use Prompt, or however you want to represent the session
                stack.Add($"{ssh.Username}@{ssh.Hostname}:{ssh.CurrentWorkingDirectory}$ ");
            }
            stack.Add($"{_session.Username}@{_session.Hostname}:{_session.CurrentWorkingDirectory}$ ");
            return stack;
        }
